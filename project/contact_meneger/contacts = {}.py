...
def menu():
    while True:
        ...
        print(
            "choose an option by presing a number or press cancel to return to the manu: "
        )
        print("1.Add contact")
...
def remove_contacts():
    while True:
        ...
        if name in contacts:
            if not yes_or_no(
                f"Are you sure you want to remove {name} from the list? (yes/no)"
            ):
                return
            del contacts[name]
    ...
    while True:
        ...
        if choice is None:
            ...
            if new_name in contacts:
                if not yes_or_no(
                    "Contact already exists — do you want to overwrite? (yes/no)"
                ):
                    continue
